...
```{r}
# Import our data
survey_responses_today <- read_sheet("https://docs.google.com/spreadsheets/d/1tVp3oanwId3hQq5adEtOzU7EkGwsfiTNQR-oDulBRQ4/edit?resourcekey#gid=390991870") %>% 
...
# Familiarity with data reasoning
...
On a 5-point scale, respondents listed their level of familiarity with data reasoning as `r number(mean_familiarity, .1)`. The figure below shows familiarity broken down by education level.
...
On a 5-point scale, respondents listed their level of interest in learning data analysis as `r number(mean_interest, .1)`.
